...
def update_planet_with_new_vals(planet_id):
    ...
    if "name" not in request_body or \
        ...
        "size" not in request_body:
            return jsonify({"message":"Request must include name, description, and size."})
    ...
    return jsonify({f"message": f"Successfully resplaced planet with ID '{planet_id}'"}), 200
...
def delete_one_planet(planet_id):
    ...
    return jsonify({"message": f"Successfully deleted bike with id `{planet_id}`"}), 200
